...
unsigned int find_nearest_vectex(const std::map<unsigned int, double>& openset){
    ...
    for(std::map<unsigned int, double>::const_iterator it = openset.begin();
            it != openset.end(); ++it){
        if(it->second < cost){
...
std::vector<unsigned int> gather_path(const std::map<unsigned int, unsigned int>& pre_vertex,
        unsigned int u, unsigned int w){
    VLOG(VERBOSITY_LOW) << "gather_path from " << u << " to " << w;
    for(std::map<unsigned int, unsigned int>::const_iterator it = pre_vertex.begin();
            it != pre_vertex.end(); it++){
...
std::vector<unsigned int> ShortestPath::path(unsigned int u, unsigned int w){
    ...
    while(openset.size() != 0){
        ...
        closeset.insert(nearest_vertex);
        ...
        for(std::vector<unsigned int>::iterator it = neighbours.begin(); it != neighbours.end(); ++it){
            ...
            double tmp_cost = 0;
            if(u == nearest_vertex)
                tmp_cost = this->_graph.get_edge_value(nearest_vertex, *it);
            else
                tmp_cost = this->_graph.get_edge_value(u, nearest_vertex) +
